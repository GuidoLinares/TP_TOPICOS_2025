...
int indice_insertar (t_indice *indice, const void *registro, size_t tamanyo,int (*cmp)(const void *, const void *))
{
  ...
  if(vectorLleno(indice))
    if(!extenderIndice(indice))
      return 0;

  
  fin = indice->vindice + (indice->cantidad_elementos_actual * sizeof(char*));
  while (fin > indice->vindice && cmp(registro, fin - (indice->cantidad_elementos_actual)) < 0)
  {
    memccpy(fin, (fin - indice->cantidad_elementos_actual), indice->cantidad_elementos_actual,sizeof(char*));
    fin -= (indice->cantidad_elementos_actual);
  }

  memccpy(fin, registro, indice->cantidad_elementos_actual,sizeof(char*));
