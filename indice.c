...
void indice_crear(t_indice *indice, size_t nmemb, size_t tamanyo)
{
    ...
    {
        // Inicializamos la estructura vac�a.
        indice->cantidad_elementos_actual = 0;
...
void indice_redimensionar(t_indice *indice, size_t nmemb, size_t tamanyo)
{
    ...
    {
        // �xito: Actualizamos el puntero y el tama�o m�ximo.
        indice->vindice = nuevo_puntero;
        indice->cantidad_elementos_maxima = nmemb;
    }
    // Si fall� (nuevo_puntero == NULL)
    // La funci�n que llam� (indice_insertar) fallar� al intentar insertar.
}

int indice_vacio(const t_indice *indice)
{
    if (indice->cantidad_elementos_actual == 0)
        return OK; // OK est� definido como 1 en indice.h
    else
        return ERROR; // ERROR est� definido como 0 en indice.h
}
...
int indice_buscar(const t_indice *indice, const void *registro, size_t nmemb, size_t tamanyo, int (*cmp) (const void *, const void *))
{
    ...
    while (izq <= der)
    {
        // 1. Calculamos la posici�n media
...
int indice_eliminar(t_indice *indice, const void *registro, size_t tamanyo, int (*cmp)(const void *, const void *))
{
    ...
    {
        // Calculamos la direcci�n de "destino" (la posici�n 'pos' que borramos)
        void *destino = p_base + (pos * tamanyo);
        // Calculamos la direcci�n de "origen" (la posici�n 'pos + 1')
        void *origen = p_base + ((pos + 1) * tamanyo);
        // Calculamos cu�ntos bytes totales hay que mover
...
{
    // Verificar si el array est� lleno y redimensionar si es necesario ---
    if (indice_lleno(indice))
    {
        unsigned nueva_maxima = (unsigned)(indice->cantidad_elementos_maxima * INCREMENTO);
        // Si el �ndice estaba vac�o (0 * 1.3 = 0), le damos el tama�o base
        if (nueva_maxima == 0)
            nueva_maxima = CANTIDAD_ELEMENTOS;

        indice_redimensionar(indice, nueva_maxima, tamanyo);
        // Si despu�s de intentar redimensionar sigue lleno, realloc() fall�.
        if (indice_lleno(indice))
            return ERROR; // No hay m�s memoria, no se puede insertar.
    }

    // Buscar la posici�n de inserci�n
    char *p_base = (char*)indice->vindice;
    int pos = 0; // Posici�n de inserci�n
    // Buscamos la posici�n 'pos' donde el elemento en esa posici�n sea MAYOR que el registro que queremos insertar.
    while (pos < indice->cantidad_elementos_actual)
    {
        ...
        if (cmp(elem_actual, registro) > 0)
            break; // Encontramos la posici�n. Hay que insertar aqu�.
        pos++;
    }
    // Si el bucle termina sin 'break', 'pos' ser� 'cantidad_elementos_actual', lo que significa que el nuevo registro va al final (es el m�s grande).
...
int indice_cargar(const char* path, t_indice* indice, void *vreg_ind, size_t tamanyo, int (*cmp) (const void *, const void *))
{
    ...
    if (arch_idx == NULL)
        return ERROR;
    // Calculamos cu�ntos registros hay en el archivo
    fseek(arch_idx, 0, SEEK_END); // Vamos al final del archivo
    long tam_archivo_bytes = ftell(arch_idx); // Obtenemos el tama�o en bytes
    rewind(arch_idx); // Volvemos al principio

    unsigned cant_registros = (unsigned)(tam_archivo_bytes / tamanyo);
    if (cant_registros == 0)
    {
        // El archivo est� vac�o, no hay nada que cargar.
        fclose(arch_idx);
        return OK;
    }
    ...
    {
        // Usamos realloc() para ajustar la memoria exactamente al tama�o necesario
